...
def search_shows():
  show_query = Show.query.options(db.joinedload(Show.Venue), db.joinedload(Show.Artist)).filter(Artist.name.ilike('%' + request.form['search_term'] + '%'))
  shows_list = list(map(Show.short, show_query))
  response = {
